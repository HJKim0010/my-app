...
function extractQuotedOrTarget(query: string): string {
  ...
  const beforeMarker = query.split(/영어로|번역|translate|into english|in english/i)[0] || query;
  const objectMatch = beforeMarker.match(/(.+?)(?:을|를)\s*(?:영어로|번역|고쳐|써|작성|바꿔|알려)/);
  return compactText(objectMatch?.[1] || beforeMarker).slice(0, 80);
}

function inferKeyPhrases(query: string, reason: RestrictionReason, language: RedirectLanguage): string[] {
  const target = extractQuotedOrTarget(query).toLowerCase();

  if (reason === "direct_translation") {
    const phrases: string[] = [];

    if (/늦|late/.test(target)) phrases.push("late");
    if (/급|서둘|hurry|rush/.test(target)) phrases.push("in a hurry");
    if (/걱정|불안|worried/.test(target)) phrases.push("worried");
    if (/두근|heart/.test(target)) phrases.push("heart was beating fast");
    if (/발견|find|found/.test(target)) phrases.push("find / discover");
    if (/잃|wallet/.test(target)) phrases.push("lose my wallet");

    return phrases.length ? phrases.slice(0, 4) : ["key action", "feeling", "reason"];
  }
  ...
  if (reason === "outside_content") {
    return language === "korean"
      ? ["story situation", "character reaction", "plausible event"]
      : ["story situation", "character reaction", "plausible event"];
  }
...
function buildFrame(reason: RestrictionReason, language: RedirectLanguage): string {
  ...
  switch (reason) {
    ...
    case "sentence_generation":
    default:
      return "먼저 ___가 일어나고, 그다음 인물은 ___를 느껴요.";
  }
}
...
function buildScaffold(
  ...
  const keyPhrases = inferKeyPhrases(query, reason, language);
  ...
  if (language === "english") {
    ...
    return {
      ...
      guidingQuestion: "What feeling, reason, or action do you want to add?",
      safeOptions: ["Share one sentence you try.", "Ask for one phrase or one flow check."],
    };
  }
  ...
  const boundaryByReason: Record<RestrictionReason, string> = {
    ...
    draft_rewrite:
      "전체 초안을 대신 고쳐 쓰기보다는, 네가 고르고 싶은 한 부분을 같이 볼게요.",
    outside_content:
      "과제와 연결되지 않는 외부 정보보다는, 이야기 안에서 자연스러운지 확인해 볼게요.",
    sentence_generation:
      "전체 이어쓰기나 문단을 대신 쓰기보다는, 네가 직접 쓸 수 있게 뼈대를 도와줄게요.",
    scoring_evaluation:
  ...
  return {
    ...
    keyPhrases,
    frame: buildFrame(reason, language),
    guidingQuestion: "어떤 감정, 이유, 행동을 넣고 싶나요?",
    safeOptions: ["네가 쓴 한 문장을 보내 주세요.", "표현 하나나 흐름 한 부분만 물어봐도 좋아요."],
  };
}
