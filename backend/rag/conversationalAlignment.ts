...
export function detectAssistantMetaFeedback(query: string): boolean {
  ...
  return /(too\s*pushy|pushing\s*too\s*much|too\s*long|too\s*wordy|only\s*answer|stop\s*asking|why\s+do\s+you\s+keep|too\s*comprehension|너무\s*푸쉬|푸시|재촉|답이\s*너무\s*길|너무\s*길|내\s*질문에만\s*답|자꾸\s*다른\s*이야기|너무\s*comprehension|이해\s*중심|자꾸\s*되묻|그만\s*물어|문법\s*설명.*필요\s*없|질문한\s*것만|묻지\s*마)/i.test(
    normalized
  );
}
...
export function buildAssistantMetaFeedbackResponse(
  ...
  if (/(답이\s*너무\s*길|너무\s*길)/i.test(normalized)) {
    return "맞아요. 답이 필요 이상으로 길었어요. 앞으로는 필요한 부분만 짧게 답하겠습니다.";
  }

  if (/(내\s*질문에만\s*답|자꾸\s*다른\s*이야기|자꾸\s*되묻|그만\s*물어|질문한\s*것만|묻지\s*마)/i.test(normalized)) {
    return "맞아요. 제가 질문 범위 밖으로 넓히거나 되물은 부분이 있었어요. 앞으로는 질문한 내용에 먼저 답하고, 불필요한 설명이나 되묻기는 줄이겠습니다.";
  }
...
export function detectAcknowledgmentOrInference(query: string): boolean {
  ...
  return /(오케이|아[,.\s]*그래서|그러면|그렇다면|그럼|so|okay|ok|then).*(중요|긴장|학생|대학생|그렇|맞|겠네|구나|군요|important|nervous|student)|^(좋아|알겠어|알겠어요|오케이|okay|ok)[.!?\s]*$/i.test(
    normalized
  );
}
...
export function buildAcknowledgmentOrInferenceResponse(
  ...
  if (/(발표|presentation|important|중요)/i.test(normalized)) {
    return language === "english"
      ...
      : "맞아요. Jack에게 발표는 중요해요. 팀이 그에게 발표 진행을 기대하고 있고, 최종 성적과 졸업에도 영향을 줄 수 있다고 나와요.";
  }
  ...
  if (/(긴장|nervous|tense|불안|초조)/i.test(normalized)) {
    return language === "english"
      ...
      : "맞아요. 그래서 긴장한 것으로 볼 수 있어요. 이미 늦었고 발표 압박이 큰데, 이상한 메시지까지 받았기 때문이에요.";
  }
  ...
  if (/(학생|대학생|student)/i.test(normalized)) {
    if (taskId === "task1") {
      return language === "english"
        ...
        : "네, 그렇게 볼 수 있어요. Jack은 학생이고, student ID, final team project presentation, final grade, graduation이 나와서 정황상 대학생으로 볼 수 있어요.";
    }
    ...
    return language === "english"
      ...
      : "그렇게 추론할 수는 있어요. 다만 원문이 Anna의 정확한 신분을 단정하진 않고, 긴 공부 뒤 laptop과 notebook을 챙기는 모습만 보여줘요.";
  }
...
export function detectContinuationStructureRequest(query: string): boolean {
  ...
  return /(내용|story|continuation|이어쓰기|다음).*(구성|구조|흐름|어떻게\s*구성|structure|organize)|(?:구성|구조|흐름).*(어떻게|좋을까|잡|story|내용|이어쓰기)/i.test(
    normalized
  );
}
